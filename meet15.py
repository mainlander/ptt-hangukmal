...
class Meet15ApplyPage(webapp.RequestHandler):
    def get(self):
        size = 0
        items = Meet15Record.all()
        for item in items:
            size += item.person_count
        template_values = {
                            "size": size
                          }
        path = os.path.join(os.path.dirname(__file__), "meet15", "apply.html")
        self.response.out.write(template.render(path, template_values))

class Meet15Add(webapp.RequestHandler):
    def post(self):
        item = Meet15Record()
...
#                            "level_str": level_str
#                          }
        self.redirect("/meet15/confirm?ident=" + item.record_hash)
#        path = os.path.join(os.path.dirname(__file__), "meet15", "confirm.html")
#        self.response.out.write(template.render(path, template_values))

class Meet15ConfirmPage(webapp.RequestHandler):
    def get(self):
        record_hash = self.request.get("ident")
        items = Meet15Record.all().filter("record_hash = ", record_hash)
        template_values = {}
        for item in items:
...
class Meet15ListPage(webapp.RequestHandler):
    def get(self):
        ...
        if user:
            if users.is_current_user_admin():
                ...
                path = os.path.join(os.path.dirname(__file__), "meet15", "list.html")
                self.response.out.write(template.render(path, template_values))
            else:
...
class Meet15SugcomPage(webapp.RequestHandler):
    def get(self):
        user = users.get_current_user()
        ...
        if user:
            if users.is_current_user_admin():
                ...
                path = os.path.join(os.path.dirname(__file__), "meet15", "sugcom.html")
                self.response.out.write(template.render(path, template_values))
